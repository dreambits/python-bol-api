...
# for Any exception generated by Bol Api response....
...
class OrderMethods(MethodGroup):
    ...
    def list(self, page=None, fulfilment_method=None,
        ...
        has_error, xml = self.request('GET', params=params,
                           accept="application/vnd.orders-v2.1+xml")

        print(xml)

        if has_error:
            return has_error, serviceError.parse(self.api, xml)
        else:
            return has_error, Orders.parse(self.api, xml)
...
class PlazaAPI(object):
    ...
    def get_has_error(self, response):
        if response.status_code in [400, 401, 500, 503]:
            return True
        else:
            return False
...
{uri}""".format(content_type=content_type,
            ...
            has_error = self.get_has_error(resp)
